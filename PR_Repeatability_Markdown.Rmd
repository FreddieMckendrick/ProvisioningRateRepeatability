...
```{r}
Prov.rate <- Prov.rate %>% left_join(NumberOfBirds) #join data sets
Prov.rate <- Prov.rate %>% 
...
Need to replace missing territory qualities with mean from previous and following Territory qualities 
...
#link territory qualities to all Field periods and territories
...
#Break up data set into summer and winter periods
...
```{r}
Prov_rate_MCMC_model1 <-MCMCglmm(ProvFeedsPerHour ~ HelperNo + Sex + GroupSize + BroodSize + Age_centred + TQ_centred, random = ~BirdID + NestID + NestID:NWID + Observer, nitt=430000, burnin=30000, thin=200, prior = prior2, verbose = TRUE, family = "poisson",  data = Prov.rate)
```
...
Some random effects are lower end so possibly could run for longer
```{r}
...
* All must pass 
Geweke diagnostic
* Checks similarity between first10% and last 50% 
...
glmer_repeatability #very similar to that produced by MCMCglmm
```
